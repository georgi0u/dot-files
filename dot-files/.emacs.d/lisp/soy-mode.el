...
(defvar template-tag-face
  (progn
    ...
    (set-face-foreground 'template-tag-face "#FC4A19")
...
(defvar html-block-tag-face
  (progn
    ...
    (set-face-foreground 'html-block-tag-face "#FCDF20")
    'html-block-tag-face))
...
(defun soy-syntactic-face-function (state)
  (let ((comment-type (nth 7 state)))  ;; see "parser state" in elisp docs
    (cond
      ((or (eq comment-type 1) (eq comment-type t))  ;; Soy tag is comment type "b"
       (cond
...
(defun soy-indent-line ()
  ...
  (let (indent-column)
    ...
    (and (< (current-column) indent-column)
         (back-to-indentation))))
